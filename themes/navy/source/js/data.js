(function() {
    ...
    axios
        .get('//now.qq.com/cgi-bin/now/activity_cms/form_data?actid=82037296')
    ...
    var renderDetailPage = function(data) {
        ...
        var renderTopicInfo = function(subjectInfo) {
            ...
            var contentTpl = '<div class="base-info"> <div> <div class="title">Talk: {{speakerItem.topic}}</div> <div class="author">{{speakerItem.name}} | {{speakerItem.brief}}</div> </div> <div class="logo-url"> <img src="{{speakerItem.avatar}}" /> </div> </div> <div class="topic-detail"> <p>{{speakerItem.intro | raw}}</p> </div> <div class="conf-info"> <div class="location"> <i></i> <div> <p class="key">Where</p> <p class="value">{{speakerItem.location}}</p> </div> </div> <div class="time"> <i></i> <div> <p class="key">Time</p> <p class="value">{{speakerItem.time}}</p> </div> </div> <div class="branch"> <i></i> <div> <p class="key">Venue</p> <p class="value">{{speakerItem.branchName}}</p> </div> </div> </div>'
            var contentOutput = swig.render(contentTpl, {
                locals: {
